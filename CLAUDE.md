...
**Current run time: ~3.25 minutes for 9 tests** (last measured 2026-08-16: `xcodebuild`'s own reported test time was 191s / 3m11s; wall-clock via `time` was 194s / 3m14s — use the larger, wall-clock figure when telling the user). Tell the user this figure (or your more recent one, see below) when you offer to run it, so they know how long to stay off the laptop — don't just say "a couple of minutes" without a number.
