...
**Current run time: ~2.6 minutes for 9 tests** (last measured 2026-09-11: `xcodebuild`'s own reported test time was 139s / 2m19s; wall-clock was 154s / 2m34s — use the larger, wall-clock figure when telling the user). It got faster than the previously recorded 3m14s because the app itself did, not because the suite changed — the tests are the same 9. Tell the user this figure (or your more recent one, see below) when you offer to run it, so they know how long to stay off the laptop — don't just say "a couple of minutes" without a number.
